...
std::shared_ptr<ImportStmt> Parser::ParseImportStmt()
{
	...
	else
	{
		...
		Lexer lexer;
		std::filesystem::path
			module_file = working_dir + "/" + node->GetFullSourceFile();

		if (std::filesystem::absolute(module_file) == std::filesystem::absolute(file))
		{
			throw std::runtime_error(
				"Circular dependency error: " + module_file.string());
		}
...
std::shared_ptr<AstNode> Parser::GetExprComponent()
{
	...
	else if (Current()->IsLiteral())
	{
		...
		{
			node =
				std::make_shared<IntLiteral>(static_cast<long>(std::stoi(Lookback()->lexeme)));
			break;
		}
		...
		{
			node =
				std::make_shared<StringLiteral>(static_cast<std::string>(Lookback()->lexeme.substr(
					1,
					Lookback()->lexeme.length() - 2)));
			break;
		}
...
std::shared_ptr<AstNode> Parser::ParseExpr(TokenType stop_token)
{
	...
	{
		if (Lookback() && (Lookback()->IsOperator()
		...
		else
		{
			...
			if (Current()->IsOperator() || Current()->token_type == TokenType::L_PARAN
				|| Current()->token_type == TokenType::R_PARAN)
			{
				...
				else if (op_stack.IsEmpty() || _prec_tbl[Current()->token_type]
					...
					<= _prec_tbl[op_stack.back()->token_type])
				{
					...
					if (op_stack.back()->IsBinaryOp())
					{
						...
						if (!op_stack.IsEmpty()
							and associativity[op_stack.back()->token_type] == LEFT)
						{
...
std::shared_ptr<ExitStmt> Parser::ParseExitStmt()
{
	...
	{
		node->exit_code =
			std::make_shared<IntLiteral>(static_cast<long>(std::stoi(Current()->lexeme)));
		AdvanceBuffer(2);
	}
...
/*****************************************************************************/
std::shared_ptr<AstNode> Parser::ParseIndexedExpr(const std::shared_ptr<AstNode>& expr)
{
	...
	std::shared_ptr<BinaryExpr> binary_expr)
{
	if (binary_expr->_op == "->" || binary_expr->_op == "::")
	{
		...
		{
			auto
				right_tmp = std::dynamic_pointer_cast<IndexedExpr>(binary_expr->_right);
